...
export const BookmarkEditModal: React.FC<BookmarkEditModalProps> = ({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!titleValidation.valid) {
            toast.error(titleValidation.error || 'Title is invalid');
            return;
        }

        const urlValidation = validateUrl(url);
        if (!urlValidation.valid) {
            toast.error(urlValidation.error || 'URL is invalid');
            return;
        }

        if (bgImage && !isValidImageUrl(bgImage)) {
            toast.error(getTranslation(appSettings.language, 'invalidImageUrl'));
            return;
        }
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs text-slate-400 font-medium">
                                                {getTranslation(appSettings.language, 'faviconUrlLabel')}
                                            </span>
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    navigator.clipboard.writeText(faviconUrl);
                                                    toast.success(getTranslation(appSettings.language, 'copiedToClipboard'));
                                                }}
                                            ...
                                            >
                                                ...
                                                {getTranslation(appSettings.language, 'copy')}
                                            </button>
                                        </div>
                                        ...
                                        <p className="text-[10px] text-slate-500 px-1">
                                            {getTranslation(appSettings.language, 'faviconTip')}
                                        </p>
                                    </div>
                                )}
                            </div>
